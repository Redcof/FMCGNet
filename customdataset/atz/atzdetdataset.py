...
class ATZDetDataset(Dataset):
    ...
    def split(self):
        df_abnormal = self.df[self.df["is_anamoly"] != 0]
    ...
    def label_transform_default(image, label, anomaly_size_px):
        ...
        if label in ["NORMAL0", "NORMAL1"] or anomaly_size_px == 0:
            return ATZDetDataset.NORMAL
        else:
            return ATZDetDataset.ABNORMAL
    ...
    def get_meta(self, idx):
        ...
        try:
            global_x1y1x2y2 = ast.literal_eval(record[["global_x1y1x2y2"]].values[0])
        except:
            global_x1y1x2y2 = None
        try:
            relative_x1y1x2y2 = ast.literal_eval(record[["relative_x1y1x2y2"]].values[0])
        except:
        ...
        return dict(current_file=current_file, label_txt=label_txt, x1=x1, x2=x2, y1=y1, y2=y2,
                    ...
                    relative_x1y1x2y2=relative_x1y1x2y2,
    ...
    def cache_limit_check(self):
        ...
        if items == ATZDetDataset.CACHE_ITEM_LIMIT:
